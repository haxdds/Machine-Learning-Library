...
public class Matrix {
    ...
    public Matrix multiply(Matrix matrix){
        if(this.columns != matrix.rows) throw new IllegalArgumentException("org.mathlib.org.mathlib.matrix.Matrix Multiplication Error: Rows must match columns");
    ...
    public Vector multiply(Vector v){
        if(this.columns != v.size() ) throw new IllegalArgumentException("org.mathlib.vector.Vector Transformation Error: org.mathlib.org.mathlib.matrix.Matrix columns must equal org.mathlib.polynomial.vector dimensions.");
    ...
    public Matrix add(Matrix matrix){
        if(!this.isEqualDimensions(matrix)) throw new IllegalArgumentException("org.mathlib.org.mathlib.matrix.Matrix Addition Error: Matrices must have equals dimensions");
        Matrix result = new Matrix(this.rows, this.columns);
            for(int i = 0; i < this.rows; i++){
                for(int j = 0; j < this.columns; j++){
    ...
    public Matrix add(Vector v){
        if(this.rows != v.size()) throw new IllegalArgumentException("org.mathlib.org.mathlib.matrix.Matrix - org.mathlib.vector.Vector Addition Error: Rows must equal org.mathlib.polynomial.vector dimensions.");
        Matrix result = new Matrix(this.rows,this.columns);
